<?php
   function manhwaclan_logs()
   {
       ...
       if(isset($_POST['manhwaclan_delete']))
       {
       ...
       if(isset($_POST['manhwaclan_delete_rules']))
       {
           $running = array();
           ...
           {
               foreach (glob(get_temp_dir() . 'manhwaclan_*') as $filename) 
               {
                  $f = fopen($filename, 'w');
                  if($f !== false)
                  {
       ...
       if(isset($_POST['manhwaclan_restore_defaults']))
       {
           manhwaclan_activation_callback(true);
       }
       if(isset($_POST['manhwaclan_delete_all']))
       {
           manhwaclan_delete_all_posts();
       }
   ?>
   ...
   <div>
      ...
      <div>
               ...
               if (!empty($running)) {
                   ...
                   foreach($running as $key => $thread)
                   {
                       foreach($thread as $param => $type)
                       {
                           $ech = '';
                           if ($type == '0') {
                               $ech .= 'GooglePlay Search Rules';
                           } elseif ($type == '1') {
                               $ech .= 'GooglePlay Channels Rules';
                           }
                           else{
         ...
         <form method="post" onsubmit="return confirm('<?php echo esc_html__('Are you sure you want to clear the running list?', 'ultimate-manga-scraper');?>');">
            <input name="manhwaclan_delete_rules" type="submit" title="<?php echo esc_html__('Caution! This is for debugging purpose only!', 'ultimate-manga-scraper');?>" value="<?php echo esc_html__('Clear Running Rules List', 'ultimate-manga-scraper');?>">
         </form>
      </div>
      <div>
         <br/>
         ...
         <div>
            ...
            <hr/>
            <form method="post" onsubmit="return confirm('<?php echo esc_html__('Are you sure you want to restore the default plugin settings?', 'ultimate-manga-scraper');?>');"><input name="manhwaclan_restore_defaults" type="submit" value="<?php echo esc_html__('Restore Plugin Default Settings', 'ultimate-manga-scraper');?>"></form>
         </div>
         <br/>
         <hr class="cr_special_hr"/>
         <div>
            ...
            <hr/>
            <form method="post" onsubmit="return confirm('<?php echo esc_html__('Are you sure you want to delete all generated posts? This can take a while, please wait until it finishes.', 'ultimate-manga-scraper');?>');"><input name="manhwaclan_delete_all" type="submit" value="<?php echo esc_html__('Delete All Generated Posts', 'ultimate-manga-scraper');?>"></form>
         </div>
         <br/>
         <hr class="cr_special_hr"/>
         <h3>
         ...
         <div>
            <?php
               if($wp_filesystem->exists(WP_CONTENT_DIR . '/manhwaclan_info.log'))
               {
                    $log = $wp_filesystem->get_contents(WP_CONTENT_DIR . '/manhwaclan_info.log');
      ...
      <form method="post" onsubmit="return confirm('<?php echo esc_html__('Are you sure you want to delete all logs?', 'ultimate-manga-scraper');?>');">
         <input name="manhwaclan_delete" type="submit" value="<?php echo esc_html__('Delete Logs', 'ultimate-manga-scraper');?>">
      </form>
   </div>
</div>
<?php
